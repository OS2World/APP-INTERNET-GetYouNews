...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszOLE = SearchFor("User-Agent: Mozilla/5.0",pszHeader, ulHeaderLen, pszOLE);
   if (pszOLE != NULL)
   {
      PSZ pszContentType;
      PSZ pszSubjectBegin;
...
/ *                                 ulDirSize = sizeof(szCurrentDir);
                                 ...
                                 if (rc == 0)
                                 {
                                    ...
                                    bMsgFound = TRUE;
/ *                                    printf("Saving Mozilla 5.0 filename %s, Encoding %s\r\n",szFileName, szEncoding);  * /
...
/ *                                    rc = DosSetCurrentDir("\\");
                                    ...
                                    if (bLogs)
                                       SaveLog("MOZILLA.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
                        ...
                        else
                        {
                           ...
                           rc = DosOpen("MOZERROR.OUT",&hfOutFile,&action,0L,FILE_NORMAL,
                              OPEN_ACTION_CREATE_IF_NEW | OPEN_ACTION_REPLACE_IF_EXISTS,
                              OPEN_SHARE_DENYNONE | OPEN_ACCESS_READWRITE,0L);
                           if (rc == 0)
                           {
                              DosWrite(hfOutFile,pszBody,
                                 ulBodyLen ,&BytesWritten);
                     ...
                     if ((*pszFileNameBegin == ' ') && (*(pszFileNameBegin - 1) == ' '))
                     {
                        ...
                        if (pszFileNameEnd != NULL)
                        {
                           ...
                           if ((pszFileNameEnd - pszFileNameBegin) < sizeof(szFileName))
                           {
                              ...
                              strcpy(szFileName,"Unknown.MOZI");
                        }
                     }
                     else
                        strcpy(szFileName,"Unknown.MOZI");
...
/*                        ulDirSize = sizeof(szCurrentDir);
                        ...
                        if (rc == 0)
                        {
                           ...
                           if (ulPart > 0)
                           {
                              sprintf(szNewFileName,"%s\\%s.Mozi%d",pszNewsGroup,szFileName, ulPart);
/*                              printf("Saving Mozilla 5.0 filename %s, part %d of %d\r\n",szFileName, ulPart,ulTotal);  */
                           }
                           else
                           {

                              sprintf(szNewFileName,"%s\\%s",pszNewsGroup,szFileName);
/*                              printf("Saving Mozilla 5.0 filename %s\r\n",szFileName);  */
                           }
                           ...
                           if (rc == 0)
                           {
                              ...
                              if (ulPart > 0)
                              {
                                 ...
                                 sprintf(szFindFile,"%s\\%s.Mozi*",pszNewsGroup,szFileName);
...
/*                                    printf("Merging Mozilla file %s\r\n", szFileName);  */
                                    ...
                                    if (rc == 0)
                                    {
                                       ...
                                       for (x = 1; x <= ulNumFiles; x++)
                                       {
                                          ...
                                          sprintf(szNewFileName,"%s\\%s.Mozi%d",pszNewsGroup, szFileName, x);
...
/*                                    printf("Saving Mozilla 5.0 filename %s, part %d of %d\r\n",szFileName, usPart, ulTotal);  */
                                    sprintf(szNewFileName,"%s\\%s.Mozi%d", pszNewsGroup,szFileName, usPart);
                                    rc = DosOpen(szNewFileName,&hfOutFile,&action,0L,FILE_NORMAL,
                                       OPEN_ACTION_CREATE_IF_NEW | OPEN_ACTION_REPLACE_IF_EXISTS,
                                       OPEN_SHARE_DENYNONE | OPEN_ACCESS_READWRITE,0L);
                                    if (rc == 0)
