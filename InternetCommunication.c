...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszUserAgent = SearchFor("X-Newsreader: Internet Communication Center v2.0.465",pszHeader, ulHeaderLen, pszUserAgent);
      ...
      if (pszSubjectBegin != NULL)
      {
         ...
         if (pszSubjectEnd != NULL)
         {
            ...
            if (pszFileMarker == NULL)
            {
               ...
               pszFileMarker = SearchFor("[Name:",pszSubjectBegin, pszSubjectEnd - pszSubjectBegin, pszFileMarker);
               if (pszFileMarker != NULL)
               {
                  pszFileMarker += 7;
...
/*                        ulDirSize = sizeof(szCurrentDir);
                        ...
                        if (rc == 0)
                        {
                           ...
                           if (ulTotal > 1)
                           {
                              sprintf(szNewFileName,"%s\\%s.ICC%d",pszNewsGroup, szFileName, ulPart);
...
/*                           rc = DosSetCurrentDir("\\");
                           ...
                           if (bLogs)
                              SaveLog("Internet Communication.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
...
/*            else
               printf("Internet Communication Response - Ignoring\r\n");  */
         }
      }
