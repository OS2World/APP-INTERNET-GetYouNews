...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszOLE = SearchFor("X-MimeOLE: Produced By Microsoft MimeOLE V4",pszHeader, ulHeaderLen, pszOLE);
   if (pszOLE != NULL)
   {
      PSZ pszContentType;
      PSZ pszSubjectBegin;
...
/*                                 ulDirSize = sizeof(szCurrentDir);
                                 ...
                                 if (rc == 0)
                                 {
                                    ...
                                    bMsgFound = TRUE;
/*                                    printf("Saving Microsoft OE 4 filename %s, Encoding %s\r\n",szFileName, szEncoding);  */
...
/*                                    rc = DosSetCurrentDir("\\");
                                    ...
                                    if (bLogs)
                                       SaveLog("OutlookExpress4.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
...
/*                                 ulDirSize = sizeof(szCurrentDir);
                                 ...
                                 if (rc == 0)
                                 {
                                    ...
                                    bMsgFound = TRUE;
/*                                    printf("Saving Microsoft OE 4 filename %s, part %d of %d\r\n",szFileName, usPart, ulTotal);  */
                                    sprintf(szNewFileName,"%s\\%s.M$4-%d", pszNewsGroup,szFileName, usPart);
