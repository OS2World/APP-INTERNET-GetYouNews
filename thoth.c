...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszUserAgent = SearchFor("User-Agent: Thoth/1.5",pszHeader, ulHeaderLen, pszUserAgent);
   if (pszUserAgent == NULL)
      pszUserAgent = SearchFor("User-Agent:  Thoth/1.5",pszHeader, ulHeaderLen, pszUserAgent);
   if (pszUserAgent == NULL)
      pszUserAgent = SearchFor("User-Agent: Thoth/1.7.2 (Carbon/OS X)",pszHeader, ulHeaderLen, pszUserAgent);
   if (pszUserAgent != NULL)
   {
      PSZ pszSubjectBegin;
      PSZ pszSubjectEnd;
      ...
      USHORT usZeros = 0;
      CHAR szThothZeros[6];
      CHAR szThothFileName[256];

      strcpy(szZeros,"00000");

/*      do
      {
         itoa(usZeros, szThothZeros, 10);
         strcpy(&szZeros[strlen(szZeros) - strlen(szThothZeros)], szThothZeros);
         sprintf(szThothFileName,"%s\\THOTH%s.OUT",pszNewsGroup,szZeros);

         rc = DosOpen(szThothFileName,&hfOutFile,&action,0L,FILE_NORMAL,
      ...
      pszUserAgentEnd = SearchFor("\r\n",pszUserAgent, ulHeaderLen - (pszUserAgent - pszHeader), pszUserAgentEnd);
      if (pszUserAgentEnd != NULL)
      {
         if (pszUserAgentEnd - pszUserAgent < 256)
         {
            strncpy(szAgent, pszUserAgent + 11, pszUserAgentEnd - pszUserAgent);
            szAgent[pszUserAgentEnd - pszUserAgent] = 0x00;
         }
         else
            strcpy(szAgent, "Thoth 1.5");
      }
      else
         strcpy(szAgent, "Thoth 1.5");
...
/*                        ulDirSize = sizeof(szCurrentDir);
                        ...
                        if (rc == 0)
                        {
                           ...
                           if (ulTotal > 1)
                           {
/*                              printf("Saving %s\r\n filename %s part %d of %d\r\n",szAgent, szFileName, ulPart, ulTotal);  */
                              sprintf(szNewFileName,"%s\\%s.Thoth%d",pszNewsGroup,szFileName, ulPart);
...
/*                              printf("Saving %s\r\n filename %s\r\n",szAgent, szFileName);  */
                           ...
                           if (rc == 0)
                           {
                              ...
                              if (ulTotal > 1)
                              {
                                 ...
                                 sprintf(szFindFile,"%s\\%s.Thoth*",pszNewsGroup,szFileName);
...
/*                                    printf("Merging %s file %s", szAgent, szFileName);  */
                                    ...
                                    if (rc == 0)
                                    {
                                       ...
                                       for (x = 1; x <= ulNumFiles; x++)
                                       {
                                          ...
                                          sprintf(szNewFileName,"%s\\%s.Thoth%d",pszNewsGroup,szFileName, x);
                           ...
                           if (bLogs)
                              SaveLog("Thoth.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
...
/*            else
               printf("THOTH reply message - Ignoring\r\n");  */
         }
      }
