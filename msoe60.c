...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszOLE = SearchFor("Produced By Microsoft MimeOLE V6.00",pszHeader, ulHeaderLen, pszOLE);
   if (pszOLE != NULL)
   {
      PSZ pszContentType;
...
/*                                    rc = DosSetCurrentDir("\\");
                                    ...
                                    if (bLogs)
                                       SaveLog("OutlookExpress60.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
...
/*                                             printf("Saving M$ OE 6.0 filename %s part %d of %d\r\n",szFileName, ulPart, ulTotal);  */
                                             sprintf(szNewFileName,"%s\\%s.MSOE60%d",pszNewsGroup, szFileName, ulPart);
                                          }
                                          else
                                          {
...
/*                                          rc = Constructor();
                                          ...
                                          if (ulTotal > 1)
                                          {
                                             ...
                                             sprintf(szFindFile,"%s\\%s.MSOE60*",pszNewsGroup,szFileName);
...
/*                                                printf("Merging M$ OE 6.0 file %s", szFileName);  */
                                                ...
                                                if (rc == 0)
                                                {
                                                   ...
                                                   for (x = 1; x <= ulNumFiles; x++)
                                                   {
                                                      ...
                                                      sprintf(szNewFileName,"%s\\%s.MSOE60%d",pszNewsGroup,szFileName, x);
...
/*                                          rc = DosSetCurrentDir("\\");
                                          rc = DosSetCurrentDir(szCurrentDir);  */
                                          if (bLogs)
                                             SaveLog("MSOE60.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
                                          if (strcmp(pszNewsGroup, szGroup) != 0)
                                             strcpy(szGroup, pszNewsGroup);
                                       }
                                    }
