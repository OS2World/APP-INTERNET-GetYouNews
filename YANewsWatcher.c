...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszUserAgent = SearchFor("User-Agent: YA-NewsWatcher/4.2.6",pszHeader, ulHeaderLen, pszUserAgent);
      ...
      if (pszSubjectBegin != NULL)
      {
         ...
         if (pszSubjectEnd != NULL)
         {
            ...
            if (pszFileMarker != NULL)
            {
               ...
               pszDataBegin = SearchFor("(This file must be converted with BinHex 4.0)",pszBody, ulBodyLen, pszDataBegin);
...
/*                     ulDirSize = sizeof(szCurrentDir);
                     ...
                     if (rc == 0)
                     {
                        ...
                        if (strlen(szFileName) > 0)
                        {
                           ...
                           if (ulTotal > 1)
                           {
/*                              printf("Saving YA-NewsWatcher filename %s part %d of %d\r\n",szFileName, ulPart, ulTotal);  */
                              sprintf(szNewFileName,"%s\\%s.YNW%d",pszNewsGroup,szFileName, ulPart);

                           }
                           else
                           {
/*                              printf("Saving YA-NewsWatcher filename %s",szFileName);  */
                           ...
                           if (rc == 0)
                           {
                              ...
                              if (ulTotal > 1)
                              {
                                 ...
                                 sprintf(szFindFile,"%s\\%s.YNW*",pszNewsGroup,szFileName);
...
/*                                    printf("Merging YA-NewsWatcher post file %s ", szFileName);  */
                                    ...
                                    if (rc == 0)
                                    {
                                       ...
                                       for (x = 1; x <= ulNumFiles; x++)
                                       {
                                          ...
                                          sprintf(szNewFileName,"%s\\%s.YNW%d",pszNewsGroup,szFileName, x);
                           ...
                           if (bLogs)
                              SaveLog("YANewsWatcher.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
