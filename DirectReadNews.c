...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszUserAgent = SearchFor("X-Newsreader: Direct Read News 2.96",pszHeader, ulHeaderLen, pszUserAgent);
   if (pszUserAgent == NULL)
      pszUserAgent = SearchFor("X-Newsreader: Direct Read News 2.97",pszHeader, ulHeaderLen, pszUserAgent);
   if (pszUserAgent == NULL)
      pszUserAgent = SearchFor("X-Newsreader: Direct Read News 4.20",pszHeader, ulHeaderLen, pszUserAgent);
   if (pszUserAgent != NULL)
   {
      PSZ pszSubjectBegin;
      PSZ pszSubjectEnd;
...
/*                        ulDirSize = sizeof(szCurrentDir);
                        ...
                        if (rc == 0)
                        {
                           ...
                           if (ulTotal > 1)
                           {
/*                              printf("Saving Direct Read News filename %s part %d of %d\r\n",szFileName, ulPart, ulTotal);  */
                              sprintf(szNewFileName,"%s\\%s.DRN%d",pszNewsGroup, szFileName, ulPart);

                           }
                           else
                           {
/*                              printf("Saving Direct Read News filename %s\r\n",szFileName);  */
...
/*                           rc = DosSetCurrentDir("\\");
                           ...
                           if (bLogs)
                              SaveLog("DirectReadNews.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
...
/*            else
               printf("Direct Read News Response - Ignoring\r\n");  */
         }
      }
