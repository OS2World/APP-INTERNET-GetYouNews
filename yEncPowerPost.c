...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszUserAgent = SearchFor("X-Newsposter: YENC-POWER-POST",pszHeader, ulHeaderLen, pszUserAgent);
   ...
   if (pszUserAgent != NULL)
   {
      ...
      if (pszUserAgentEnd != NULL)
      {
         if (pszUserAgentEnd - pszUserAgent < 256)
         {
            ...
            strcpy(szAgent, "YENC-POWER-POST");
      }
      else
         strcpy(szAgent, "YENC-POWER-POST");
         ...
         if (pszSubjectEnd != NULL)
         {
            ...
            if (pszReply == NULL)
            {
               ...
               if (pszFileMarker != NULL)
               {
                  ...
                  if ((pszFileMarker1 != pszSubjectEnd) && (pszFileMarker1 != NULL))
                  {
...
/*                              printf("Saving %s\r\n filename %s part %d of %d\r\n",szAgent, szFileName, ulPart, ulTotal);  */
                              sprintf(szNewFileName,"%s\\%s.yEncPP%d",pszNewsGroup,szFileName, ulPart);
...
/*                              printf("Saving %s\r\n filename %s\r\n",szAgent, szFileName);  */
                           ...
                           if (rc == 0)
                           {
                              ...
                              if (ulTotal > 1)
                              {
                                 ...
                                 sprintf(szFindFile,"%s\\%s.yEncPP*",pszNewsGroup,szFileName);
...
/*                                    printf("Merging %s file %s", szAgent, szFileName);  */
                                    ...
                                    if (rc == 0)
                                    {
                                       ...
                                       for (x = 1; x <= ulNumFiles; x++)
                                       {
                                          ...
                                          sprintf(szNewFileName,"%s\\%s.yEncPP%d",pszNewsGroup,szFileName, x);
                           ...
                           if (bLogs)
                              SaveLog("yEncPowerPost.LOG",pszHeader, ulHeaderLen, pszNewsGroup, strcmp(pszNewsGroup, szGroup));
...
/*            else
               printf("yEnc Power Post reply message - Ignoring\r\n");  */
         }
      }
