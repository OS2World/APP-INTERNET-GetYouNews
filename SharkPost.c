...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   x = _dllentry;
   pszNEWSPoster = SearchFor("X-Newsposter: SharkPost v1.0",pszHeader, ulHeaderLen, pszNEWSPoster);
   if (pszNEWSPoster != NULL)
   {
      ...
      *pszTemp = 0x00;
      if (strncmp(szVersion,"SharkPost v1.0", 14 ) == 0)
         strcpy(szVersion,"SharkPost10.log");
...
/*      Constructor();  */
      ...
      if (pszSubjectBegin != NULL)
      {
         ...
         if (pszSubjectEnd != NULL)
         {
            ...
            if (pszFileMarker1 > pszFileMarker)
            {
...
/*                  ulDirSize = sizeof(szCurrentDir);
            ...
            if (rc == 0)
            {
               ...
               if (ulTotal > 1)
                  sprintf(szNewFileName,"%s\\%s.SHKP%d",pszNewsGroup, szFileName, ulPart);
               else
               ...
               if (rc == 0)
               {
                  ...
                  if (ulTotal > 1)
                  {
                     ...
                     sprintf(szFindFile,"%s\\%s.SHKP*",pszNewsGroup, szFileName);

                     rc = DosFindFirst( szFindFile,&hdirFindHandle,FILE_NORMAL,&FindBuffer,
                     ...
                     if (ulNumFiles == ulTotal)
                     {
                        ...
                        if (rc == 0)
                        {
                           ...
                           for (x = 1; x <= ulNumFiles; x++)
                           {
                              ...
                              sprintf(szNewFileName,"%s\\%s.SHKP%d",pszNewsGroup, szFileName, x);
