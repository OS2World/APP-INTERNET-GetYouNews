...
BOOL CheckForMultipart(PSZ pszHeader, ULONG ulHeaderLen, PSZ pszBody, ULONG ulBodyLen, PSZ pszNewsGroup, BOOL bLogs)
{
   ...
   pszUserAgent = SearchFor("User-Agent: NewsPro/3.5.1",pszHeader, ulHeaderLen, pszUserAgent);
      ...
      else
      {
         ...
         if (pszSubjectBegin != NULL)
         {
            ...
            if (pszSubjectEnd != NULL)
            {
               ...
               strcpy(szLogFileName, "NewsPro.Log");
               pszFileMarker = NULL;
               pszFileMarker = SearchFor(" - ",pszSubjectBegin, pszSubjectEnd - pszSubjectBegin, pszFileMarker);
               while (pszFileTemp != NULL)
               {
                  pszFileMarker = pszFileTemp;
                  pszFileTemp = SearchFor(" - ",pszFileMarker + 1, pszSubjectEnd - (pszFileMarker + 1), pszFileTemp);
               }
               ...
               if ((strlen(szFileName) > 0) && (ulPart > 0))
               {
                  ...
                  if (ulTotal > 1)
                  {
                     sprintf(szNewFileName,"%s\\%s.NewsPro%d",pszNewsGroup, szFileName, ulPart);
                  }
                  else
                  {
                     if (ulPart > 0)
                  ...
                  if (ulPart > 0)
                  {
                     ...
                     if (rc == 0)
                     {
                        ...
                        if (ulTotal > 1)
                        {
                           ...
                           sprintf(szFindFile,"%s\\%s.NewsPro*",pszNewsGroup,szFileName);
                           ...
                           if (ulNumFiles == ulTotal)
                           {
                              ...
                              if (rc == 0)
                              {
                                 ...
                                 for (x = 1; x <= ulNumFiles; x++)
                                 {
                                    ...
                                    sprintf(szNewFileName,"%s\\%s.NewsPro%d",pszNewsGroup,szFileName, x);
